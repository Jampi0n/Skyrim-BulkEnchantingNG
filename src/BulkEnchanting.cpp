...
namespace BulkEnchanting {
    ...
    public:
        ...
        void AddFlexible(Count num) { flexible += num; }
    ...
    bool IsReusable(TESSoulGem* soulGem) { return soulGem->HasKeywordID(FormID(0xed2f1)); }
    ...
    std::string GetEnchantmentKey(ExtraDataList* subgroup) {
        ...
        if (extraEnch != NULL) {
            ...
            for (auto effect : enchantmentItem->effects) {
                ...
                s += "_eff=[" + hexId + "]" + effect->baseEffect->GetFullName() +
                     "_mag=" + std::to_string(effect->GetMagnitude()) + "_dur=" + std::to_string(effect->GetDuration());
            }
        }
        return s;
    }
    ...
    public:
        ...
        static void ListItems(std::string message, CountMap<TESBoundObject*> unenchanted,
                              CountMap<std::string> enchantments, CountMap<SOUL_LEVEL> soulGems,
                              CountMap<ExtraDataList*> enchantedSubgroups) {
            if (spdlog::default_logger().get()->level() <= spdlog::level::level_enum::trace) {
    ...
    item type			optional		 optional list for every sub group within item type	   data specific for this
    sub group

    TESBoundObject -> InventoryEntryData -> BSSimpleList<ExtraDataList> -> ExtraDataList -> BSExtraData (e.g.
    Enchantment) |				 -> BSExtraData (e.g. Soul Level) |				 -> BSExtraData (e.g. Tempering)
                                                                        |
                                                                        -> ExtraDataList
                                                                        -> ExtraDataList
                                                                        -> ExtraDataList
    ...
    The inventory is stored in a map using TESBoundObject* as key type. Item forms (Weapons, Armor, Potions, ...) are
    inherit from TESBoundObject. The key can also be understood to use the FormID of the items. In order to
    differentiate sub groups of items with the same FormID, extra data is used. An item type (FormID) can have a list of
    extra data lists (BSSimpleList<ExtraDataList>). Each node in the BSSimpleList refers to one sub group of the item
    type. Base items do not have nodes in this BSSimpleList. So if the player has one normal iron dagger and one
    enchanted and simultaneously tempered iron dagger, the TESBoundObject for the iron dagger will have a
    InventoryEntryData with a BSSimpleList<ExtraDataList> with one node. The ExtraDataList will have two nodes, one to
    store the enchantment and one to store the tempering. As there are two iron daggers in total, but only one node in
    the BSSimpleList, the remaining iron dagger has no extra data and is a default iron dagger. A sub group can have
    multiple items that share their extra data. In that case, the count is stored as another node in the ExtraDataList
    of this sub group.
    ...
    */
    ...
    public:
        ...
        CountMap<std::string> enchantments;  // Player made enchantments (extra data) of items with at most Enchantment
                                             // and Name as extra data
        CountMap<SOUL_LEVEL>
            soulGems;  // Soul levels used to determine which soul was used to potentially trigger bulk enchanting
    ...
    private:
        ...
        void AddEnchanted(ExtraDataList* item, Count num) { enchantedSubgroups.modItem(item, num); }
                    ...
                    if (key->formType == FormType::SoulGem) {
                                ...
                                for (auto subgroup : *extraLists) {
                                    if (SubgroupGetSoul(subgroup) != NULL) {
                                        ...
                                        if (dynamicLevel != SOUL_LEVEL::kNone && defaultLevel == SOUL_LEVEL::kNone) {
                                            ...
                                            // the subgroup argument is required to refer to it later, when choosing
                                            // which groups to empty in case of reusable soul gems, as the groups have
                                            // be emptied as a whole, so the correct subgroups need to be chosen to
                                            // achieve the maximum number of souls
                                            AddSoulSize(dynamicLevel, subgroup->GetCount(), subgroup,
                                                        IsReusable(soulGem), true);
                        ...
                        if (remaining > 0) {
                            ...
                            if (defaultLevel != SOUL_LEVEL::kNone) {
                                ...
                                // if they are reusable, they cannot be used for bulk enchanting but can still trigger
                                // it (because they would keep their soul when bulk enchanting) YASTM
                                // (https://www.nexusmods.com/skyrimspecialedition/mods/56144) contains statically
                                // filled reusable soul gems
                        ...
                        if (!IsEnchanted(
                                key)) {  // base item must not be enchanted, so it can be enchanted by the player
                            int remaining = num;
                            if (entryData != NULL) {
                                auto extraLists = entryData->extraLists;
                                if (extraLists != NULL) {
                                    for (auto subgroup : *extraLists) {
                                        ...
                                        // any other extra data is avoided, as these are the two extra datas that are
                                        // added by enchanting
    ...
    Count OnItemEnchanted(StaticFunctionTag*, Actor* player) {
        ...
        if (usedSouls.isSingular() && newEnchantments.isSingular() && removedItems.isSingular()) {
            // After enchanting, one soul and one unenchanted item should be gone, while one enchanted item should
            // appear If this is the case, the entire enchantment process is known and can be repeated
            ...
            if (unique.find(lastItem) != unique.end()) {
                ...
                if (num > 0) {
                    logger::debug(
                        "You have {} {}'s. The following cannot be enchanted (this should always include the item that "
                        "was just enchanted):",
                        num, lastItem->GetName());
                    ...
                    if (entryData != NULL) {
                        ...
                        if (extraLists != NULL) {
                            for (auto subgroup : *extraLists) {
                                remaining -= subgroup->GetCount();
                                logger::debug("    {} items cannot be enchanted because of this extra data:",
                                              subgroup->GetCount());
                                for (auto& xData : *subgroup) {
                                    auto xType = xData.GetType();
                                    logger::debug("        [{}]: {}", ExtraDataTypeToString(xType),
                                                  ExtraDataTypeToInfo(xType));
                                }
                            }
                        }
                    }
            ...
            auto possibleRepeats =
                currentPlayerInventory.soulGemCount.at(static_cast<Count>(lastSoul)).GetBestVector(maxRepeats).first;

            if (possibleRepeats < maxRepeats) {
                logger::debug("Only {} of the {} soul gems can be used for bulk enchanting.", possibleRepeats,
                              maxRepeats);
                logger::debug("  1.Reusable soul gems (e.g. Azura's Star) are emptied instead of being removed.");
                logger::debug(
                    "    Stacked groups can only be emptied as a group, so groups can only be used for bulk "
                    "enchanting, if the number of enchanted items is large enough.");
                logger::debug(
                    "    Newly filled soul gems are generally not stacked and are only stacked once they are moved to "
                    "chest and back.");
                logger::debug(
                    "    Note that directly enchanting a stacked reusable soul gem removes souls from the entire group "
                    "(vanilla bug), so trigger the bulk enchanting with a normal soul of the same size to make use of "
                    "the stacked souls.");
                logger::debug(
                    "    You cannot unstack the group without losing the souls. Dropping or removing a portion of the "
                    "stacked group, removes the souls from that portion (vanilla bug).");
                logger::debug(
                    "  2.Reusable soul gems which are filled by default (e.g. Azura's Star in "
                    "YASTM:https://www.nexusmods.com/skyrimspecialedition/mods/56144) cannot be used for bulk "
                    "enchanting.");
                logger::debug(
                    "    Note that directly enchanting a stacked reusable soul with YASTM does work correctly and can "
                    "trigger bulk enchanting.");
            }
            ...
            if (!usedSouls.isSingular()) {
                ...
                logger::debug("Soul size: {}", static_cast<int>(usedSouls.getSingular()));
            }
            if (!removedItems.isSingular()) {
                logger::debug("Cannot use bulk enchanting, because enchanted item is invalid.");
                ...
                if (enchantedSubgroups.isSingular()) {
                    ...
                    for (auto& xData : *subgroup) {
                        if (!hasExtraData) {
                            logger::debug("Enchanted item is invalid, because it has the following extra data:");
                            hasExtraData = true;
                        }
                    ...
                    if (!hasExtraData) {
                        logger::trace("Enchanted item has no extraData.");
                    }
                } else {
                    logger::trace("Enchanted item could not be determined.");
                }
            } else {
                logger::debug("Unenchanted Item: {}", removedItems.getSingular()->GetName());

                if (!newEnchantments.isSingular()) {
                    logger::debug("Cannot use bulk enchanting, because used enchantment could not be determined.");
                } else {
                    logger::debug("Enchantment: {}", newEnchantments.getSingular());
                }
            }
    ...
    Count RepeatEnchantment(StaticFunctionTag*, Actor* player, Count repeat) {
        logger::trace("RepeatEnchantment({}, {}, {}) x {}", lastItem->GetName(), lastEnchantment,
                      static_cast<int>(lastSoul), repeat);
        ...
        if (soulGemInfo.first < repeat) {
            logger::error("Trying to repeat ({}) more often than there are valid soul gems ({}).", repeat,
                          soulGemInfo.first);
        ...
        if (!foundEnchantment) {
            ...
            logger::info(
                "    This may be caused by our weapons or armor being removed from the outside while enchanting.");
            return 0;
        }
                    ...
                    if (remainingSouls > 0) {
                                    ...
                                    if (soulLevel == lastSoul) {
                                        ...
                                        if (IsReusable(soulGem)) {
                                            ...
                                            if (remove < subgroup->GetCount()) {
                                                // Cannot set soul for only a subset of the subgroup, continue with next
                                                // subgroup
                                            } else {
                                                // Remove soul from subgroup
                                                // This should usually not happen, as all possible reusable groups have
                                                // already been used and the remaining ones should be too large.
                                                auto extraSoul = SubgroupGetSoul(subgroup);
                                                if (extraSoul == NULL) {
                                                    logger::error("Soul gem is already empty.");
                                                } else {
                                            ...
                                            // The RemoveItem function works with a subgroup argument to remove items
                                            // from a specific subgroup (even partially)
                                            player->RemoveItem(key, remove, ITEM_REMOVE_REASON::kRemove, subgroup,
                                                               nullptr);
                                            remainingSouls -= remove;
                                        }
                                    }
                        ...
                        if (remaining > 0) {
                            if (soulGem->GetContainedSoul() == lastSoul) {
                                ...
                                if (IsReusable(soulGem)) {
                                    // If the soul gem is reusable and has no extra data it's just an infinitely full
                                    // soul gem Ignore, these soul gems are not supported for bulk enchanting and there
                                    // should be enough left that are supported
                                } else {
